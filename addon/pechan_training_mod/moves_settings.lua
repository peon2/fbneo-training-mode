...
PECHAN_CONFIG.REVERSAL_MOVES.MOVELIST = movelist
reversal_types = {
	GUARD = "GUARD",
	WAKEUP = "WAKEUP",
	HIT = "HIT"
...
local function fillVarNames(type)
	for key, item in pairs(moves) do
		if moves[key].default == true then
			table.insert(PECHAN_CONFIG.MOVES_VAR_NAMES[type], key)
			PECHAN_CONFIG.MOVES_VAR_NAMES[type][key] = PECHAN_CONFIG.REVERSAL_MOVES.OPTIONS.ON
		else
			table.insert(PECHAN_CONFIG.MOVES_VAR_NAMES[type], key)
			PECHAN_CONFIG.MOVES_VAR_NAMES[type][key] = PECHAN_CONFIG.REVERSAL_MOVES.OPTIONS.OFF
		end
	end
end
...
local GUARD_GUI = require "addon.pechan_training_mod.move_data.guard_gui"
...
-- added normal moves to default page
local WAKEUP_GUI = require "addon.pechan_training_mod.move_data.wakeup_gui"
...
-- added normal moves to default page
local HIT_GUI = require "addon.pechan_training_mod.move_data.hit_gui"
...
function getIndexFromConfigValue(value)
	for index, confValue in pairs(PECHAN_CONFIG.TRAINING.CONFIGURATIONS) do
...
function getCurrentReversalMoves(type)
	...
	for index, value in pairs(PECHAN_CONFIG.MOVES_VAR_NAMES[type]) do
		if (PECHAN_CONFIG.MOVES_VAR_NAMES[type][index] == PECHAN_CONFIG.REVERSAL_MOVES.OPTIONS.ON) then
...
function deactivateAllDefaultMoves()
	for index, value in pairs(PECHAN_CONFIG.MOVES_VAR_NAMES[reversal_types.GUARD]) do
...
function resetAllConfiguration()
	...
	PECHAN_CONFIG.RECOVERY.recovery = PECHAN_CONFIG.RECOVERY.OPTIONS.OFF

	PECHAN_CONFIG.RECOVERY.delay = 10
	PECHAN_CONFIG.RECOVERY.times = 8
end

local function enableReversalMove(move_name, type)
	PECHAN_CONFIG.MOVES_VAR_NAMES[type][move_name] = PECHAN_CONFIG.REVERSAL_MOVES.OPTIONS.ON
end
-- Function to set default configuration based on configName
function setDefaultConfig(configName)
	resetAllConfiguration()

	if configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["None"] then
		return
	end
	if configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["cd_pressure_1"] then
		-- activate recovery
		...
		PECHAN_CONFIG.RECOVERY.delay = 24
		PECHAN_CONFIG.RECOVERY.times = 3
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["cd_pressure_2"] then
		-- activate recovery
		PECHAN_CONFIG.RECOVERY.dummy_recovering = true
		PECHAN_CONFIG.RECOVERY.recovery = PECHAN_CONFIG.RECOVERY.OPTIONS.ON
		--set recovery time for CD knockdown
		...
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
		printTable(PECHAN_CONFIG.WAKEUP.reversal_moves)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["cd_pressure_3"] then
		-- activate recovery
		PECHAN_CONFIG.RECOVERY.dummy_recovering = true
		PECHAN_CONFIG.RECOVERY.recovery = PECHAN_CONFIG.RECOVERY.OPTIONS.ON
		--set recovery time for CD knockdown
		...
		-- reload reversal moves
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["cd_pressure_4"] then
		-- activate recovery
		PECHAN_CONFIG.RECOVERY.dummy_recovering = true
		PECHAN_CONFIG.RECOVERY.recovery = PECHAN_CONFIG.RECOVERY.OPTIONS.ON
		--set recovery time for CD knockdown
		...
		-- reload reversal moves
		PECHAN_CONFIG.GUARD.reversal_moves = getCurrentReversalMoves(reversal_types.GUARD)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["crouching_frametrap"] then
		--activate guard reversal
		PECHAN_CONFIG.GUARD.dummy_guarding = true
		PECHAN_CONFIG.GUARD.reversal = PECHAN_CONFIG.GUARD.REVERSAL_OPTIONS.ON
		--activate crouch guard
		...
		-- reload reversal moves
		PECHAN_CONFIG.GUARD.reversal_moves = getCurrentReversalMoves(reversal_types.GUARD)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["standing_frametrap"] then
		--activate guard reversal
		PECHAN_CONFIG.GUARD.dummy_guarding = true
		PECHAN_CONFIG.GUARD.reversal = PECHAN_CONFIG.GUARD.REVERSAL_OPTIONS.ON
		--activate crouch guard
		...
		-- reload reversal moves
		PECHAN_CONFIG.GUARD.reversal_moves = getCurrentReversalMoves(reversal_types.GUARD)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["high_confirm_against_CDA"] then
		--activate guard reversal
		PECHAN_CONFIG.GUARD.dummy_guarding = true
	...
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["wakeup_whiff_cr_c"] then
		...
		-- reload reversal moves
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["wakeup_dpc"] then
		--activate wake up random
		PECHAN_CONFIG.WAKEUP.dummy_waking_up = true
		PECHAN_CONFIG.WAKEUP.reversal = PECHAN_CONFIG.WAKEUP.REVERSAL_OPTIONS.ON
		-- activate dp on wakeup
		...
		-- reload reversal moves
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["shimmy_wakeup"] then
		--activate wake up random
		PECHAN_CONFIG.WAKEUP.dummy_waking_up = true
		PECHAN_CONFIG.WAKEUP.reversal = PECHAN_CONFIG.WAKEUP.REVERSAL_OPTIONS.RANDOM
		-- activate dp on wakeup
		...
		-- reload reversal moves
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["wakeup_delay_OS_basic"] then
		--activate wake up random
		PECHAN_CONFIG.WAKEUP.dummy_waking_up = true
		PECHAN_CONFIG.WAKEUP.reversal = PECHAN_CONFIG.WAKEUP.REVERSAL_OPTIONS.RANDOM
		-- activate dp on wakeup
		...
		-- reload reversal moves
		PECHAN_CONFIG.WAKEUP.reversal_moves = getCurrentReversalMoves(reversal_types.WAKEUP)
	elseif configName == PECHAN_CONFIG.TRAINING.CONFIGURATIONS["wakeup_delay_OS_full"] then
		--activate wake up random
		PECHAN_CONFIG.WAKEUP.dummy_waking_up = true
		PECHAN_CONFIG.WAKEUP.reversal = PECHAN_CONFIG.WAKEUP.REVERSAL_OPTIONS.RANDOM
		-- activate dp on wakeup
