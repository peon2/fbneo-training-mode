...
local harness = require("addon.pechan_training_mod.tests.test_harness")

-- Globals expected by pechan_training_mod
-- We assume pechan_training_mod has already been required once to load its globals (PECHAN_CONFIG, functions)
...
function TestGuard:setUp()
    ...
    -- Reset PECHAN_CONFIG to defaults
    PECHAN_CONFIG.GUARD.guard_mode = 0
    PECHAN_CONFIG.GUARD.dummy_action = 0 -- Standing

    -- Reset state machine if exposed, or ensure transitionToState works via mocks
    -- Since state variables are local to pechan_training_mod.lua, we might need to reset them
    ...
    -- Important: Addresses used in pechan_training_mod.lua (must match the file)
...
function TestGuard:testOneHitGuard_Trigger()
    -- Config: 1 Hit Guard
    PECHAN_CONFIG.GUARD.guard_mode = 4
    ...
    -- Looking at pechan_training_mod.lua: Run() calls checkFrameAdvantage and infiniteTime,
    ...
    -- Line 1191: "if PECHAN_CONFIG.GUARD.guard_mode == 4 then" is inside block().
