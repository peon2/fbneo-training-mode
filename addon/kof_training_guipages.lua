...
local hit_reversal_move_active_settings = {
	title = {
...
--[[
-- Link static Hit Reversal tables from move_data to guipages
-- This matches the user's request for static data arrays.
local status, loaded_move_data = pcall(require, "addon.kof_training_move_data")
if status and loaded_move_data then
    guipages.hit_command_normals = loaded_move_data.hit_command_normals
    guipages.hit_specials = loaded_move_data.hit_specials
    guipages.hit_supers = loaded_move_data.hit_supers
    guipages.hit_normals = loaded_move_data.hit_normals
    guipages.hit_commons = loaded_move_data.hit_commons
    -- Recordigns are usually handled differently or embedded, but if they exist as pages:
    -- guipages.hit_recordings = loaded_move_data.hit_recordings -- (if needed)
end
--]]
