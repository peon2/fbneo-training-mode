...
require("games/ssf2t/character_specific")
require("games/ssf2t/gamestate")
require("games/ssf2t/constants")
...
local function loadReplayConfig()
	if REPLAY then
		if not replay_config then
			dofile("games/ssf2t/customconfig_replay.lua")
			replay_config = true
		end
	else
...
local function shiftP1(status)
	if status then
		inputs.properties.scrolling.P1.x = 30
		inputs.properties.scrolling.P1.y = 100
	else
		inputs.properties.scrolling.P1.x = 3
		inputs.properties.scrolling.P1.y = 100
	end
end
...
local function shiftP2(status)
	if status then
		inputs.properties.scrolling.P2.x = 285
		inputs.properties.scrolling.P2.y = 100
	else
		inputs.properties.scrolling.P2.x = 310
		inputs.properties.scrolling.P2.y = 100
	end
end
...
local function reversalThrow(_throw)
	...
	if reversal_throw_ready then
		...
		if playerTwoFacingLeft() then
				setHoldDirection({"Right", determineThrowInput(_throw)})
			else
				setHoldDirection({"Left", determineThrowInput(_throw)})
			end
		return
...
local function frameTrapAnalysis(_player_obj)
	...
	local y = 100
	if attacker.id == 1 then
		x = inputs.properties.scrolling.P1.x + 90
	elseif attacker.id == 2 then
		x = inputs.properties.scrolling.P2.x - 90
	end
