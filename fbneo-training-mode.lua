...
guiinputs = {
	P1 = { previousinputs = {}, coinframestart = 0, coinpresscount = 0, leftframecount = 0, rightframecount = 0,
		downframecount = 0, upframecount = 0, },
	P2 = { previousinputs = {} },
	KB = { inputcount = {} },
}
...
do -- file checking logic + global variables/tables
	...
	if nbuttons == 0 then
		...
		nbuttons = nil
		for b = 6, 1, -1 do
			for _, v in ipairs({ "P1 Button " .. b, "P1 Button " .. string.char(b + 64), "P1 Fire " .. b, "P1 " .. a[b], "P1 " ..
			a2[b] }) do                                                                                                    -- some common buttons
				if c[v] ~= nil then
					nbuttons = b
					break
				end
...
local meterHandlerP1 = function()
	...
	if combovars.p2.combo ~= combovars.p2.previouscombo and not modulevars.p2.inhitstun then
		if combovars.p1.refillmeter == 0 then
			combovars.p1.refillmeter = math.ceil((modulevars.p1.maxmeter - modulevars.p1.meter) / combovars.p1
			.refillmeterspeed)                                                                                                -- refill speed
		end
	end
...
local meterHandlerP2 = function()
	...
	if combovars.p1.combo ~= combovars.p1.previouscombo and not modulevars.p1.inhitstun then
		if combovars.p2.refillmeter == 0 then
			combovars.p2.refillmeter = math.ceil((modulevars.p2.maxmeter - modulevars.p2.meter) / combovars.p2
			.refillmeterspeed)                                                                                                -- refill speed
		end
	end
...
local menuCheck = function()
	interactivegui.inmenu = interactivegui.enabled or interactivegui.movehud.enabled or interactivegui.replayeditor
	.enabled
...
setDirection = function(player, ...)                                                                                          -- getting a player to hold down/up etc.
	...
	if type(dir1) == "number" then                                                                                              -- for numpad
		local a = { { "Down", "Left" }, { "Down" }, { "Down", "Right" }, { "Left" }, {}, { "Right" }, { "Up", "Left" },
			{ "Up" }, { "Up", "Right" } }                                                                                           -- numpad
		dir2 = a[dir1][2]
		dir1 = a[dir1][1]
	end
...
local buttonHandler = function(t)
	...
	if helpElements.name == nil or t.funcs.name ~= helpElements.name then -- new set of buttons, should be checking using enums
		...
		for i = 1, helpElements.len do
			helpElements[i] = { name = t[i].name, button = buttonHandlerInputs[t[i].button], buttonnum = t[i].button,
				func = t[i].func }
		end
...
local drawInteractiveGUI = function()
	...
	local t = { { name = "SLCT", button = "button1" }, { name = "HKEY", button = "button2" }, {},
		funcs = drawInteractiveGUIFuncs }
	...
	gui.box(selection.x + boxx, selection.y + boxy, selection.x + boxx + w + 4, selection.y + boxy + h, selection.bgcolour,
		colour)
...
local drawReplayEditorFuncs = {
	...
	function(but)                                                                                                              -- swap directions
		...
		if guiinputs.P1[but] and not guiinputs.P1.previousinputs[but] then                                                       -- invert direction
			...
			interactivegui.replayeditor.inputs[recording.recordingslot][interactivegui.replayeditor.editframe].p2facingleft
	...
	other = function()
		if interactivegui.replayeditor.framestart then return end
		if guiinputs.P1.upframecount ~= 0 then interactivegui.replayeditor.editframe = interactivegui.replayeditor.editframe -
			hudworkingframes(guiinputs.P1.upframecount) end
...
local drawReplayEditor = function()
	...
	t = { { name = "SET" }, { name = "COPY" }, { name = "CLR" }, { name = "DEL" }, { name = "BLNK" }, { name = "<NUM" },
		{ name = "NUM>" }, { name = "SWP" }, funcs = drawReplayEditorFuncs }
	buttonHandler(t)
...
local readHotkeyIn = function()
	...
	if interactiveguipages[interactivegui.page][interactivegui.selection].info then t[1] = { name = "INFO",
			button = "button1" } end
	...
	if availablefunctions.readplayertwohealth then
		table.insert(HUDElements,
			{ name = "p2health",
				...
				drawfunc = function() gui.text(hud.p2healthx, hud.p2healthy, modulevars.p2.health, hud.p2healthtextcolour) end })
		if availablefunctions.playertwoinhitstun then
		...
		table.insert(HUDElements,
			{
				...
				x = function(n)
					if n then changeConfig("inputs", "scrollinginputxoffset", { n, scroll.scrollinginputxoffset[2] }, scroll) end
					return scroll.scrollinginputxoffset[1]
				end,
				y = function(n)
					if n then changeConfig("inputs", "scrollinginputyoffset", { n, scroll.scrollinginputyoffset[2] }, scroll) end
				...
				movehud = {                                                                                                                                                                                                                                                                                                  -- extra functions for scrolling input
					{ name = "NUMS",
						               func = function(but) if guiinputs.P1[but] and not guiinputs.P1.previousinputs[but] then
					...
					{ name = "INC",
						               func = function(but) if guiinputs.P1[but] and not guiinputs.P1.previousinputs[but] then if inputs.properties.scrollinginput.iconsize < 16 then
					...
					{ name = "DEC",
						               func = function(but) if guiinputs.P1[but] and not guiinputs.P1.previousinputs[but] then if inputs.properties.scrollinginput.iconsize > 8 then
									changeConfig("inputs", "iconsize", inputs.properties.scrollinginput.iconsize - 1,
										inputs.properties.scrollinginput)
									scrollingInputReload()
								end end end },                                                                                                                                                                                                                                                                                       -- increase size of text (prone to crashing)
		...
		table.insert(HUDElements,
			{
				...
				x = function(n)
					if n then changeConfig("inputs", "scrollinginputxoffset", { scroll.scrollinginputxoffset[1], n }, scroll) end
					return scroll.scrollinginputxoffset[2]
				end,
				y = function(n)
					if n then changeConfig("inputs", "scrollinginputyoffset", { scroll.scrollinginputyoffset[1], n }, scroll) end
