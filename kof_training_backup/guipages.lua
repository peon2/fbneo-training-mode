guicustompage = {
	...
	},
	{
		...
		handle = 9,
		func = function()
			if KOF_CONFIG.get_current_game().name == "The King of Fighters '98" then
...
for page = 1, total_pages do
	...
	table.insert(page_table, {
		...
		func = function()
			...
			if not KOF_CONFIG.UI.CURRENT_PLAYER1 or not KOF_CONFIG.UI.CURRENT_PLAYER2 then
				return
			end
			if current_game.has_strikers then
				if not KOF_CONFIG.UI.P1_STRIKER1 or not KOF_CONFIG.UI.P2_STRIKER1 then
					return
				end
				if current_game.has_3_strikers then
					if not KOF_CONFIG.UI.P1_STRIKER2 or not KOF_CONFIG.UI.P1_STRIKER3 or
						not KOF_CONFIG.UI.P2_STRIKER2 or not KOF_CONFIG.UI.P2_STRIKER3 then
						return
					end
				end
			end
