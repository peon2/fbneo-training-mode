...
require("addon.kof_training.config")

local rom_name = emu.romname() or "unknown"
if KOF_CONFIG.SUPPORTED_GAMES[rom_name] then
	print("KOF Training Mode: Activated for '" .. rom_name .. "'")
else
	return
end
...
if not _G.kof_training_tests_run then
	...
	-- Load suites
	require("addon.kof_training.tests.test_guard")
...
local AI = require("addon.kof_training.ai.init")
local Trials = require("addon.kof_training.ai.trials")
local Cinematics = require("addon.kof_training.ai.cinematics")
...
function applySetup(setup)
	...
	if setup.wakeup then
		...
		for i = 1, 5 do
			...
			KOF_CONFIG.REVERSAL_MOVES.MOVELIST:getReversal("REC_" .. i).propagates = setup.recording_var_states[i]
				.propagates
		end
	...
	if setup.hit then
		resetReversals(KOF_CONFIG.MOVES_VAR_NAMES, "HIT")
		dumpTable(KOF_CONFIG.MOVES_VAR_NAMES["HIT"])
		for i = 1, 5 do
			if setup.recording_var_states[i] then
				KOF_CONFIG.MOVES_VAR_NAMES["HIT"]["REC_" .. i] = setup.recording_var_states[i].value
				KOF_CONFIG.REVERSAL_MOVES.MOVELIST:getReversal("REC_" .. i).propagates = setup.recording_var_states[i]
					.propagates
			end
		end
	...
	if (CURRENT_REVERSAL_MOVE_NAME) == nil then
		if state == "guard_reversal" then
			if KOF_CONFIG.GUARD.reversal == KOF_CONFIG.GUARD.REVERSAL_OPTIONS.RANDOM then
				CURRENT_REVERSAL_MOVE_NAME = KOF_CONFIG.GUARD.reversal_moves
					[math.random(1, #KOF_CONFIG.GUARD.reversal_moves)]
			else
		...
		elseif state == "waking_up" then
			if KOF_CONFIG.WAKEUP.reversal == KOF_CONFIG.WAKEUP.REVERSAL_OPTIONS.RANDOM then
				CURRENT_REVERSAL_MOVE_NAME = KOF_CONFIG.WAKEUP.reversal_moves
					[math.random(1, #KOF_CONFIG.WAKEUP.reversal_moves)]
			else
		...
		elseif state == "hit_reversal" then
			if KOF_CONFIG.HIT.reversal == KOF_CONFIG.HIT.REVERSAL_OPTIONS.RANDOM then
				CURRENT_REVERSAL_MOVE_NAME = KOF_CONFIG.HIT.reversal_moves
					[math.random(1, #KOF_CONFIG.HIT.reversal_moves)]
			else
				CURRENT_REVERSAL_MOVE_NAME = KOF_CONFIG.HIT.reversal_moves[1]
			end
...
local function block()
	...
	if KOF_CONFIG.GUARD.guard_mode == KOF_CONFIG.GUARD.MODE_OPTIONS.OFF then
		if KOF_CONFIG.DEBUG.BLOCK == 1 then
			gui.text(10, 50, "Mode: OFF")
		end
		-- Should have transitioned to start, but if here safely:
		if KOF_CONFIG.GUARD.dummy_action == KOF_CONFIG.GUARD.ACTION_OPTIONS.CROUCHING then p2Crouch() end
		return
	end

	-- 2. Check Guard Mode: ON
	if KOF_CONFIG.GUARD.guard_mode == KOF_CONFIG.GUARD.MODE_OPTIONS.ON then
		if KOF_CONFIG.DEBUG.BLOCK == 1 then
	...
	if KOF_CONFIG.GUARD.guard_mode == KOF_CONFIG.GUARD.MODE_OPTIONS.RANDOM then
		if KOF_CONFIG.DEBUG.BLOCK == 1 then
	...
	if KOF_CONFIG.GUARD.guard_mode == KOF_CONFIG.GUARD.MODE_OPTIONS.ALL_GUARD then
		if KOF_CONFIG.DEBUG.BLOCK == 1 then
...
function setDefaultConfig(configName)
	if configName == "safe_jump_training" then
		...
		KOF_CONFIG.WAKEUP.reversal_moves       = KOF_CONFIG.REVERSAL_MOVES.getCurrentReversalMoves("WAKEUP")
...
local function draw_distance_status(character1, character2)
	...
	if KOF_CONFIG.DEBUG.DISTANCE == 1 then
...
local function draw_action_code(character, x, y)
	...
	if KOF_CONFIG.DEBUG.ACTION == 1 then
...
local function draw_stun_status(character1, character2)
	...
	x = 200
	y = 48
	if KOF_CONFIG.DEBUG.STUN == 1 then
...
local function draw_frame_advantage()
	...
	if KOF_CONFIG.DEBUG.ADVANTAGE == 1 then
...
local function draw_debug_info()
	...
	if KOF_CONFIG.DEBUG.METER == 1 then
	...
	if emu.romname and emu.romname() == "kof2001" then
		gui.text(10, 150,
			string.format("P1(10A7E6):%02X  S1:%02X  S2:%02X  S3:%02X", rb(0x10A7E6), rb(0x10A7E7), rb(0x10A7E8),
				rb(0x10A7E9)), "yellow")
		gui.text(10, 160, string.format("P1 Inf Strikers (1082E2): %02X", rb(0x1082E2)), "yellow")
		gui.text(10, 170,
			string.format("P2(10A804):%02X  S1:%02X  S2:%02X  S3:%02X", rb(0x10A804), rb(0x10A805), rb(0x10A806),
				rb(0x10A807)), "cyan")
		gui.text(10, 180, string.format("P2 Inf Strikers (1084E2): %02X", rb(0x1084E2)), "cyan")
	end
