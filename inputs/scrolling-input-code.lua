...
local icon_size, image_icon_size = inputs.properties.scrolling.iconsize
...
local resourcepath = SCROLLING_INPUT_RESOURCEPATH .. "32/"
local recordpath = recordpath .. "/"
local emu = emu or gens
----------------------------------------------------------------------------------------------------
...
local display = image
if not fexists(resourcepath .. iconfile) then
	print("Icon file " .. resourcepath .. iconfile .. " not found.")
...
local function readimages()
	...
	if display == image then
		...
		image_icon_size = sourceimg:sizeX();
...
function togglescrollinginputsplayer()
	draw[1] = inputs.properties.scrolling.P1.enabled
	draw[2] = inputs.properties.scrolling.P2.enabled
end
...
function scrollingInputReg()
	...
	if inputs.properties.scrolling.P1.x == -1 then
		setConfigDefault("scrollinginputxp1", math.floor(margin[1]))
		resetConfig("scrollinginputxp1")
	end
	if inputs.properties.scrolling.P2.x == -1 then
		setConfigDefault("scrollinginputxp2", math.floor(margin[2]))
		resetConfig("scrollinginputxp2")
	end
	if inputs.properties.scrolling.P1.y == -1 then
		setConfigDefault("scrollinginputyp1", math.floor(margin[3]))
		resetConfig("scrollinginputyp1")
	end
	if inputs.properties.scrolling.P2.y == -1 then
		setConfigDefault("scrollinginputyp2", math.floor(margin[3]))
		resetConfig("scrollinginputyp2")
	end

	local x = { inputs.properties.scrolling.P1.x, inputs.properties.scrolling.P2.x }
	local y = { inputs.properties.scrolling.P1.y, inputs.properties.scrolling.P2.y }

	for player = 1, 2 do
		local xoffset = x[player]
		local yoffset = y[player]
		if draw[player] then
			local i = 0
			local skip = 0
