...
--TODO COLOURS AS A CONFIG OPTION
...
local function generic() --try to detect controls and make a generic module
	...
	local stick,nbuttons,nplayers,label = 0,0,2
	...
	--for n=4,1,-1 do if c["P"..n.." Button 1"] ~= nil or c["P"..n.." Fire 1"] ~= nil then nplayers = n break end end -- training mode only targets 2-player games
	if stick+nbuttons == 0 then inputDisplayReg=false return end -- found neither stick nor buttons, can't use this
	
	print("Generic input: "..nplayers.."-player, "..nbuttons.."-button"..(stick > 0 and "" or ", no joystick")) print()
	module = {}
	if stick > 0 then
		for n=1,nplayers do
...
local function displayfunc()
	...
	for k,v in pairs(module) do
		...
		if string.sub(k, 1, 1)=="2" then -- only working for two players
			if inputs.properties.simple.P2.enabled then
				gui.text(inputs.properties.simple.P2.x+(v[1]-module["2c"][1]), inputs.properties.simple.P2.y+(v[2]-module["2^"][2]), string.sub(k, 2), color1, color2)
			end
		else
			if inputs.properties.simple.P1.enabled then
				gui.text(inputs.properties.simple.P1.x+(v[1]-module["1c"][1]), inputs.properties.simple.P1.y+(v[2]-module["1^"][2]), string.sub(k, 2), color1, color2)
			end
		end
	end
end
...
function inputDisplayReg()
	...
	if inputs.properties.simple.P1.x == -1 then
		changeConfig("simpleinputxp1", module["1c"][1])
	end
	if inputs.properties.simple.P2.x == -1 then
		changeConfig("simpleinputxp2", module["2c"][1])
	end
	if inputs.properties.simple.P1.y == -1 then
		changeConfig("simpleinputyp1", module["1^"][2])
	end
	if inputs.properties.simple.P2.y == -1 then
		changeConfig("simpleinputyp2", module["2^"][2])
	end
