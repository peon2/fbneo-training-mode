-- peon2: don't know the original author of this script, but I think it's Mountainmanjed, edited to work with https://github.com/peon2/fbneo-training-mode/
...
local attkcolor = {0xFF,0x00,0x00,0x00}
local blckcolor = {0xFF,0xFF,0xFF,0x00}
local hurtcolor = {0x77,0x77,0xFF,0x00}
local prjecolor = {0xEE,0x00,0xDD,0x00}
local pushcolor = {0x00,0xFF,0x00,0x00}

local a,g,v,p = attkcolor,blckcolor,hurtcolor,prjecolor
...
local function drawaxis(x,y,axis)
	gui.line(x+axis,y,x-axis,y,"white")
	gui.line(x,y+axis,x,y-axis,"white")
end

local function colbox(plx,ply,adr,flp,active)
	...
	top = y + h
	btm = y - h
	if active ~= 0 and color then
		gui.box(lft,top,rgt,btm,color)
		--gui.text(x-8,y,id,0xFFFFFFFF,0x000000FF)
	end
end

local function data(dadr)--Quick hack job to get this done
...
local function pushbox(plx,ply,adr,color)
