...
local rb, rbs, rw, rws, rd, fc = memory.readbyte, memory.readbytesigned, memory.readword, memory.readwordsigned,
		memory.readdword, emu.framecount
...
local profile = {
	{
		...
		breakpoints       = {
...

			{
				base = 0x00EBE4,
				no_background = true,
				func = function()                                   --no BG *** setregister for m68000.pc is broken ***
					memory.setregister("m68000.pc", gr("pc") - 0x2)
				end
			},
		},
	...
	},
	{
		...
		breakpoints       = {
			...
			},
			{
				base = 0x0263BC,
				base = 0x013EFE,
				func = function()                              --jubei/laurence airthrow
		...
		remove_background = function()
			memory.writebyte(0x100920, 0x20)
			memory.writebyte(0x10092B, 0x3C)                              --HUD
	...
	},
	{
		...
		breakpoints       = {
...

			{
				base = 0x0249BA,
				func = function()             --ground throws
					insert_throw({ ground = bit.band(0xFFFF, gr("d0")) })
				end
			},
			{
				base = 0x025AD0,
				func = function()             --air throws
					insert_throw({ air = gr("d0"), offset = gr("a1") })
				end
			},
			{
				base = 0x024210,
				func = function()             --special throw param
					insert_throw({ tier = bit.band(0xFF, gr("d0")) + 1 })
				end
			},
			{
				base = 0x030BF0,
				func = function()             --jubei _B,F+LK
					insert_throw({ tier = rw(gr("pc") + 0x2) })
				end
			},
			{
				base = 0x030C98,
				func = function()             --jubei _B,F+HK
					insert_throw({ tier = rw(gr("pc") + 0x2) })
				end
			},
			{
				base = 0x036BE6,
				func = function()             --ryo DM
					local pc = gr("pc")
					insert_throw({
						tier = rw(pc + 0x2) + 1,
						hard_top = rws(pc + 0xC),
	...
	},
	{
		...
		breakpoints       = {
			...
			{
				base = 0x0681EC,
				func = function()             --airthrows
					insert_throw({ ptr = gr("a0") })
				end
			},
			{
				base = 0x044ED4,
				func = function()             --sokaku 360
					insert_throw({ range = gr("d7") })
				end
			},
			{
				base = 0x04555C,
				func = function()             --mary 360
					insert_throw({ range = gr("d7") })
				end
			},
			{
				base = 0x0474B0,
				func = function()             --andy spider hold
					...
					insert_throw({ right = range, left = -range, top = 0 })
				end
			},
			{
				base = 0x052B88,
				func = function()             --mary spider hold
					local alt = rws(gr("a4") + 0x28)
					local pc = gr("pc")
					if alt > rws(pc + 0x34) or alt < rws(pc + 0x3E) then
						return
					end
					insert_throw({ right = rws(pc + 0x2C), top = 0 })
				end
			},
			{
				base = 0x053B72,
				func = function()             --mary DM/SDM
					local alt = rws(gr("a4") + 0x28)
					local pc = gr("pc")
					if alt > rws(pc + 0x34) or alt < rws(pc + 0x3E) then
						return
...

			{
				base = 0x003718,
				no_background = true,
				func = function()                                   --solid shadows
					memory.setregister("m68000.d0", 1)
				end
			},
		},
		remove_background = function()
	...
	},
	{
		...
		breakpoints       = {
			...
			{
				base = 0x040ED2,
				func = function()             --sokaku 360; duck dm/sdm; yamazaki sdm
					insert_throw({ range = bit.band(0xFFFF, gr("d7")) })
				end
			},
			{
				base = 0x043CD6,
				func = function()             --andy spider throw
					local pc = gr("pc")
					local alt = rws(gr("a4") + 0x28)
			...
			{
				base = 0x04EC5C,
				func = function()             --mary spider throw
					local pc = gr("pc")
					local alt = rws(gr("a4") + 0x28)
			...
			{
				base = 0x050C0C,
				func = function()             --mary dm
					insert_throw({ range = rws(gr("pc") + 0x34) })
				end
			},
			{
				base = 0x051136,
				func = function()             --mary sdm
...

			{
				base = 0x003758,
				["rbff1a"] = 0,
				no_background = true,                             --solid shadows
	...
	},
	{
		...
		breakpoints       = {
			...
			{
				base = 0x07299C,
				func = function()                                    --check vuln at all times *** setregister for m68000.pc is broken ***
					memory.setregister("m68000.pc", gr("pc") + 0x6)
				end
			},
			{
				base = 0x07299C,
				func = function()                                    --check vuln at all times *** hackish workaround ***
					memory.setregister("m68000.a3", gr("a3") - 0xB5)
				end
			},
			{
				base = 0x0729A2,
				func = function()                                    --*** fix for hackish workaround ***
					memory.setregister("m68000.a3", gr("a3") + 0xB5)
				end
			},
			...
			{
				base = 0x045660,
				func = function()             --special ground throw
					insert_throw({ range = gr("d7") })
				end
			},
			{
				base = 0x0544C6,
				func = function()             --ex mary snatcher
					local pc = gr("pc")
					insert_throw({
						range_x = rws(pc + 0x12),
						range_y = rws(pc + 0x28),
	...
	},
	{
		...
		breakpoints       = {
			...
			},
			{
				base = 0x035212,
				["garoup"] = -0x20C,
				["garoubl"] = -0x20C,                                  --check vuln at all times *** setregister for m68000.pc is broken ***
				func = function() memory.setregister("m68000.pc", gr("pc") + 0x6) end
			},
			{
				base = 0x035212,
				["garoup"] = -0x20C,
				["garoubl"] = -0x20C,                                  --check vuln at all times *** hackish workaround ***
				func = function() memory.setregister("m68000.a3", gr("a3") - 0xB7) end
			},
			{
				base = 0x035218,
				["garoup"] = -0x20C,
				["garoubl"] = -0x20C,                                  --*** fix for hackish workaround ***
				func = function() memory.setregister("m68000.a3", gr("a3") + 0xB7) end
			},
			...
			},
			{
				base = 0x012B56,
				["garoup"] = -0x312,
				["garoubl"] = -0x312,                                  --p1 push
				func = function() insert_box(4, 2) end
			},
			{
				base = 0x012B84,
				["garoup"] = -0x31C,
				["garoubl"] = -0x31C,                                  --p2 push
				func = function() insert_box(3, 1) end
			},
...
local update_hitboxes = function()
	...
	f.screen_left = rws(game.stage_base + offset.pos_x) +
	(320 - emu.screenwidth()) / 2                                                      --FBA removes the side margins for some games
...
local draw_hitbox = function(hb, same_plane)
	...
	if globals.draw_mini_axis then
		...
		gui.drawline(hb.hval, hb.vval - globals.mini_axis_size, hb.hval, hb.vval + globals.mini_axis_size,
			boxes[hb.type].outline)
		gui.drawline(hb.hval - globals.mini_axis_size, hb.vval, hb.hval + globals.mini_axis_size, hb.vval,
			boxes[hb.type].outline)
